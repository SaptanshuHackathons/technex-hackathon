...
const MODELS: Model[] = [
    { id: "gemini-flash", name: "Gemini Flash", tier: "free", credits: 1 },
...
export function URLInput({ className }: { className?: string }) {
    ...
    const [enableDeepScrape, setEnableDeepScrape] = React.useState(false);
    const [maxDepth, setMaxDepth] = React.useState(1);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            // Redirect to chat page with scraping params
            const params = new URLSearchParams({
                scraping: 'true',
                url: url,
                enableDeepScrape: enableDeepScrape.toString(),
    ...
    return (
        <>
            <form
                ...
                className={cn(
                    "relative flex w-full max-w-2xl flex-col gap-3",
                    className
                )}
            >
                {/* Main input row */}
                <div className="flex items-center gap-2 rounded-2xl border border-black/5 bg-white p-2 shadow-xl transition-all hover:shadow-2xl focus-within:ring-4 focus-within:ring-black/5 dark:border-white/10 dark:bg-black dark:focus-within:ring-white/10">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            disabled={isLoading}
                            className={cn(
                                "flex h-10 items-center gap-2 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 px-4 text-sm font-semibold text-white transition-all hover:from-emerald-700 hover:to-emerald-800 disabled:opacity-50",
                ...
                {/* Deep scraping controls */}
                <div className="flex items-center justify-between rounded-xl border border-black/5 bg-gray-50/50 px-4 py-2.5 dark:border-white/5 dark:bg-zinc-900/50">
                    <div className="flex items-center gap-3">
                        <label className="flex items-center gap-2 cursor-pointer">
                            <div className="relative">
                                <input
                                    type="checkbox"
                                    checked={enableDeepScrape}
                                    onChange={(e) => setEnableDeepScrape(e.target.checked)}
                                    className="peer sr-only"
                                />
                                <div className="h-5 w-9 rounded-full bg-gray-300 peer-checked:bg-emerald-600 transition-colors dark:bg-zinc-700" />
                        ...
                        {enableDeepScrape && (
                            <div className="flex items-center gap-2">
                                ...
                                <select
                                    ...
                                    className="rounded-lg border border-black/5 bg-white px-2 py-1 text-sm font-medium outline-none transition-colors hover:border-emerald-500/30 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 dark:border-white/10 dark:bg-zinc-800 dark:text-white"
                                >
                    ...
                    {enableDeepScrape && (
                        <span className="text-xs text-gray-400 dark:text-gray-500">
                            Discover and index linked pages
                        </span>
                    )}
                </div>
            </form>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                    <div className="w-full max-w-md rounded-2xl border border-black/5 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-zinc-900">
                        ...
                        <input
                            ...
                            className="w-full rounded-xl border border-black/5 bg-gray-50 px-4 py-3 text-sm outline-none transition-all focus:ring-2 focus:ring-emerald-500/20 dark:border-white/10 dark:bg-zinc-800 dark:text-white"
                        />
                        ...
                        <div className="mt-6 flex gap-3">
                            ...
                            <button
                                ...
                                disabled={!apiKey}
                                className="flex-1 rounded-xl bg-gradient-to-r from-emerald-600 to-emerald-700 px-4 py-2.5 text-sm font-medium text-white transition-colors hover:from-emerald-700 hover:to-emerald-800 disabled:opacity-50"
                            >
                                Save
                            </button>
                        </div>
